...
export default async function LoginPage({ searchParams }: PageProps<"/login">) {
  ...
  if (!result.ok) {
    return (
      <LoginShell heading="Not configured" lede="This server cannot authenticate anyone yet.">
  ...
  if (config.mode === "google") {
    return (
      <LoginShell heading="Sign in" lede="Continue with the Google account this instance was opened for.">
        ...
        <a
          href={`/api/auth/start?next=${encodeURIComponent(next)}`}
          className="ui-btn ui-btn--primary ui-login__cta"
        >
          <GoogleMark />
          Continue with Google
        </a>
        <p className="ui-login__foot">
          Only allowlisted Google accounts are admitted. Everyone else is turned away
          after the Google prompt, not before it.
        </p>
      </LoginShell>
    );
  }
  ...
  return (
    <LoginShell
      heading="Sign in"
      lede="The key is checked against Hevy and matched to an allowlisted account id."
    >
      {errorMessage && <p className="ui-notice ui-notice--warn">{errorMessage}</p>}
